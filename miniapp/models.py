...
class Category(models.Model):
    ...
    def get_fields_for_filter_in_template(self):
        return ProductsFeatures.objects.filter(
            category=self,
            use_in_filter=True
...
class ProductsFeatures(models.Model):
    ...
    postfix_for_value = models.CharField(
        ...
        help_text=f'Например для характеристики "Время(часы) горения" к значению можно добавить постфикс "часов", и '
        f'как результат - значение "2 часа" '
    )
...
class ProductFeatureValidators(models.Model):
    category = models.ForeignKey(Category, verbose_name='Категория', on_delete=models.CASCADE)
    feature = models.ForeignKey(ProductsFeatures, verbose_name='Характеристика', null=True, blank=True,
